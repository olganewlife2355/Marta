...
const CONFIG = {
  ...
  // Посилання — замінити на реальні перед публікацією форми!
  LINKS: {
    ogranka: '[ПОСИЛАННЯ НА ОПЛАТУ ОГРАНКИ]',        // напр., LiqPay
    pidpyska: '[ПОСИЛАННЯ НА ОФОРМЛЕННЯ ПІДПИСКИ]',  // напр., Telegram
    consultation: '[ПОСИЛАННЯ НА ЗАПИС НА КОНСУЛЬТАЦІЮ]', // напр., Calendly
  },

  DATES: {
    ogrankaStart: '[ДАТА СТАРТУ ОГРАНКИ]', // напр., «понеділок, 4 серпня»
  },

  PRICES: {
    ogranka: '1500 грн',
    pidpyska: '1000 грн/місяць',
  },

  SIGNATURE:
    '\n\n—\nЛюдмила Бойко\n' +
    'Instagram: [ВАШ ІНСТАГРАМ]\n' +
...
const CHOICE = {
  ...
  PIDPYSKA: '📱 ДІАНА. Підписка — щомісячний супровід, ' + CONFIG.PRICES.pidpyska,
  FULL: '👑 ДІАНА. Повна програма — глибока персональна робота (вартість обговорюємо на консультації)',
...
function setupSurveyForm() {
  ...
  form.addPageBreakItem()
    ...
    .setHelpText(
      ...
      '📱 ДІАНА. Підписка — щомісячний супровід: практики, ефіри, ' +
      'спільнота. Гнучко і без довгих зобов\'язань. Вартість: ' +
      CONFIG.PRICES.pidpyska + '.\n\n' +
      '👑 ДІАНА. Повна програма — глибока персональна робота зі мною ' +
      'до результату. Формат і вартість обговорюємо на консультації.'
    );
...
function sendPersonalizedEmail(data) {
  ...
  if (data.program === CHOICE.OGRANKA) {
    ...
    body =
      ...
      'Забронювати місце: ' + CONFIG.LINKS.ogranka + '\n\n' +
      'Якщо є запитання — просто відпиши на цей лист.';

  } else if (data.program === CHOICE.PIDPYSKA) {
    subject = '📱 ДІАНА. Підписка — твій формат';
    body =
      firstName + ', привіт!\n\n' +
      'Дякую за твої відповіді!\n\n' +
      'Ти обрала підписку ДІАНА — гнучкий формат, який легко ' +
      'вписати навіть у щільний графік.\n\n' +
      'Що входить у підписку:\n' +
      '• щомісячні практики та завдання;\n' +
      '• живі ефіри та відповіді на запитання;\n' +
      '• закрита спільнота підтримки;\n' +
      '• доступ до матеріалів у зручний для тебе час.\n\n' +
      'Вартість: ' + CONFIG.PRICES.pidpyska + '\n' +
      'Приєднатися можна вже сьогодні: ' + CONFIG.LINKS.pidpyska + '\n\n' +
      'Якщо є запитання — просто відпиши на цей лист.';

  } else if (data.program === CHOICE.FULL) {
    subject = '👑 ДІАНА. Повна програма — твоя трансформація';
    body =
      firstName + ', привіт!\n\n' +
      'Дякую за відвертість у відповідях — це важливо.\n\n' +
      (data.dream
        ? 'Ти написала про свою мрію на 2026 рік:\n«' + data.dream + '»\n\n' +
          'Повна програма ДІАНА — саме про те, щоб пройти цей шлях ' +
          'не самій, а з персональним супроводом до результату.\n\n'
        : 'Повна програма ДІАНА — це глибока персональна робота ' +
          'зі мною до результату.\n\n') +
      'Що на тебе чекає:\n' +
      ...
      'і я розповім, як може виглядати твоя програма:\n' +
      CONFIG.LINKS.consultation + '\n\n' +
      'До зустрічі!';
    ...
    body =
      ...
      'Пропоную безплатну 15-хвилинну зустріч: розкажеш про свою ' +
      'ситуацію, а я підкажу, з чого краще почати саме тобі.\n\n' +
      'Обрати зручний час: ' + CONFIG.LINKS.consultation + '\n\n' +
      'Або просто відпиши на цей лист — відповім особисто.';
  }
